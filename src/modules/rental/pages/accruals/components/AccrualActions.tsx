...
export default function AccrualActions({
  statusId,
  permissions,
  editing,
  loading,
  ...
  if (editing)
    return (
      <Space wrap>
        ...
        <Button icon={<X className="size-4" />} onClick={onDiscard}>
          {t("common.cancel")}
        </Button>
      </Space>
    );
  return (
    <Space wrap>
      {isRentalDraft(statusId) && can(rentalAccrualPermissions.update) && (
        <Button icon={<Pencil className="size-4" />} onClick={onEdit}>
          {t("common.edit")}
        </Button>
      )}
      {isRentalDraft(statusId) && can(rentalAccrualPermissions.post) && (
        <Popconfirm
          title={t("rental.actions.postConfirm")}
          onConfirm={onPost}
          okText={t("rental.actions.post")}
        ...
        >
          <Button
            ...
            loading={loading}
          >
            {t("rental.actions.post")}
          </Button>
        </Popconfirm>
      )}
      {(isRentalDraft(statusId) || isRentalPosted(statusId)) &&
        can(rentalAccrualPermissions.cancel) && (
          <Popconfirm
      ...
      {isRentalDraft(statusId) && can(rentalAccrualPermissions.delete) && (
        <Popconfirm
