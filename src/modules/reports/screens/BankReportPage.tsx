...
import { bankDirectionOptions } from "../constants/options";
import { reportManualEndpoints } from "../constants/endpoints";
import { operationalReportPermissions } from "../constants/permissions";
import type { BankOperationReportRow } from "../types/type";

type Row = BankOperationReportRow & { indexId: number };

const isIncome = (row: BankOperationReportRow) => row.directionId > 0;
...
export default function BankReportPage() {
  ...
  const columns: TableColumnsType<Row> = [
    { dataIndex: "indexId", title: t("common.rowNumber"), width: 64 },
    {
      ...
      render: (value, record) => (
        <Link to={`/main/bank/${record.id}`}>{value || record.id}</Link>
      ),
    },
    {
      dataIndex: "bankDocumentNumber",
      title: t("reports.fields.bankDocNumber"),
      render: (value) => value || "—",
    },
    {
      dataIndex: "docDate",
    ...
    },
    {
      ...
      title: t("reports.fields.bankAccount"),
      render: (value, record) => (
  ...
  return (
    <ReportListPage<BankOperationReportRow>
      report="bank"
      exportPermission={operationalReportPermissions.bank.export}
      columns={columns}
      filters={
        <>
          <SearchFilter />
          <SelectFilter
            ...
            placeholder="reports.fields.bankAccount"
            path={reportManualEndpoints.orgBankAccounts}
            search
            width={220}
          />
          <SelectFilter
            paramKey="directionId"
            placeholder="reports.fields.direction"
            options={bankDirectionOptions}
            width={160}
          />
          <DateRangeFilter
