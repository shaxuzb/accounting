...
const assetSchema = (t: TFunction) => yup.object().shape({
  ...
  salvageValue: yup.number().min(0, t("fa.validation.salvageValueNonNegative")).required(t("fa.validation.salvageValueRequired")),
...
export const faReceiptSchema = (t: TFunction) => yup.object().shape({
  ...
  receiptType: yup.string().required(t("fa.validation.receiptTypeRequired")),
