export * from "./routes";
export * from "./pages/faAsset/constants/permissions";
export * from "./pages/faDisposal/constants/permissions";
export * from "./pages/faDepreciation/constants/permissions";
export * from "./pages/faMovement/constants/permissions";
export * from "./pages/faReceipt/constants/permissions";
export * from "./pages/faRevaluation/constants/permissions";
